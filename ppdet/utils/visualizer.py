...
def draw_bbox(image, im_id, catid2name, bboxes, threshold):
    """
    Draw bbox on image with fixed colors for each category.
    """
    ...
    for dt in np.array(bboxes):
        ...
        # 调整类别ID以适应从1开始的索引
        adjusted_catid = catid

        # 检查调整后的类别ID是否在颜色列表范围内
        if adjusted_catid < 0 or adjusted_catid >= len(color_list):
            logger.error(f'Adjusted category ID {adjusted_catid} is out of range for color list.')
            continue

        # 为每个类别ID分配固定颜色
        color = tuple(int(color_list[adjusted_catid][i:i+2], 16) for i in (1, 3, 5))
