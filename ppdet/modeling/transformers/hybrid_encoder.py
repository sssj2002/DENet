...
class CSPRepLayer(nn.Layer):
    def __init__(self,
        ...
        self.conv2 = BaseConv(
            in_channels, hidden_channels, ksize=1, stride=1, bias=bias, act=act)
        self.conv4 = ESF(
            hidden_channels, hidden_channels,stride=1)
...
class BranchWithPDC(nn.Layer):

    def __init__(self, in_channels, out_channels, ksize, stride=1, theta=0.5, act="silu"):
        super().__init__()
        ...
        nn.initializer.KaimingUniform()(self.conv.weight)
    ...
    def forward(self, x):
        ...
        if abs(self.theta) > 1e-8:
            kernel_diff = W.sum(axis=(2, 3), keepdim=True) 
            out_diff = F.conv2d(
                x, kernel_diff, stride=self.stride,
                padding=0, dilation=1, groups=1
            )
            y_raw = out_normal - self.theta * out_diff
        else:
            y_raw = out_normal
        y = self.act(self.bn(y_raw))
        return y
...
class ESF(nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, stride=1, act="silu"):
        ...
        self.branch1 = BranchWithPDC(in_channels, out_channels, ksize=7, stride=1, theta=0.5, act=act)
        self.branch2 = BranchWithPDC(in_channels, out_channels, ksize=5, stride=1, theta=0.5, act=act)
        self.branch3 = BranchWithPDC(in_channels, out_channels, ksize=3, stride=1, theta=0.5, act=act)
        self.branch4 = BranchWithPDC(in_channels, out_channels, ksize=3, stride=1, theta=0.5, act=act)

        self.conv_final = BaseConv(
    ...
    def forward(self, x):
        ...
        x3 = self.branch3(x)
        x4 = self.branch4(x)
        ms = paddle.concat([x1, x2, x3, x4], axis=1)
        out = self.conv_final(ms)
        return out
